...
>(({ className, ...props }, ref) => (
    <TabsPrimitive.Trigger
        ref={ref}
        className={cn(
            'group flex items-center gap-1.5 whitespace-nowrap rounded-lg p-2.5 font-medium transition-all focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50 hover:bg-light-theme hover:text-black data-[state=active]:bg-light-theme data-[state=active]:text-black [&>svg]:size-[18px] [&>svg]:shrink-0 [&[data-state=active]>svg]:text-primary',
            className,
        )}
        {...props}
    />
